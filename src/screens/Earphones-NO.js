...
import { dataEarphone } from "../data/dataEarphone";
...
const Circle = ({ scrollX }) => {
    ...
    return (
        <View style={[StyleSheet.absoluteFillObject, styles.circleContainer]}>
            {
                dataEarphone.map(({ color, key }, index) => {
                    ...
                    return (
                        <Animated.View key={key} style={[styles.circle, { backgroundColor: color, transform: [{ scale }], opacity }]}>
    ...
    return (
        <View style={styles.tickerContainer}>
            <Animated.View style={{
                ...
                {
                    dataEarphone.map(({ type, key }) => {
                        return (
                            <Text key={key} style={styles.tickerText}>{type}</Text>
                        )
...
const EarphoneCard = ({ imageUri, heading, description, index, scrollX }) => {
    ...
    return (
        <View style={styles.itemStyle}>
            <Animated.Image source={imageUri} style={[styles.imageStyle, {
                transform: [{ scale }]
            }]} />
    ...
    return (
        <View style={styles.container}>
            ...
            <Animated.FlatList
                ...
                data={dataEarphone}
                renderItem={({ item, index }) => <EarphoneCard {...item} index={index} scrollX={scrollX} />}
                pagingEnabled
